...
def check_reference_files(genome):
    ...
    for refpath in [gtf_path, chrom_name_path, chrom_start_path, tx_path, ex_path]:
        if not os.path.exists(refpath):
            sys.exit("[error] Annotation file do not exists: {} !".format(refpath))
        else:
...
def tagging_reads(genome, bam, outpath):
    ...
    **tagging_reads** will read the bam file which sorted by header or barcode, report the UMIs and reads counts for
    ...
    #gtf path
    gtf_path = os.path.join(star_index_dir, "genes", "genes.gtf")
    ...
    with open("/home/ref/genes/genes.gtf", "r") as infile:
        for line in infile:
            cells = line.split()
            if len(cells)<10:
                continue
    ...
    print("[info] Tagging bamfile to gene expression info: {}".format(outpath))
